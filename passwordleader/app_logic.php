...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
    ...
    if (mysqli_num_rows($results) == 1) {
      ...
      header('location: ../passwordleader/success.php');
    }else {
...
if (isset($_POST['reset-password'])) {
  ...
  if (count($errors) == 0) {
    ...
    $msg = "Hi there, click on this <a href=\"localhost/badgesys/passwordleader/new_password.php?email=".$email."&token=".$token."\">link</a> to reset your password on our site";
    $msg = wordwrap($msg,70);
    ...
    header('location: ../passwordleader/pending.php?email=' . $email);
  }
}
...
if (isset($_GET['token']) != $tokens) {
  header('location: ../passwordleader/expire.php?email=' . $email);
  } 
} 
...
if (isset($_POST['new_password'])) {
  ...
  if ($time_expire > $current_time) {
    ...
    if ($email) {
        ...
        header('location: ../passwordleader/success.php?email=' . $email);
        }        
      }       
  } else {
    $deleterecord = "DELETE FROM password_reset WHERE token = '$token'";
