...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
    ...
    $sql = "SELECT * FROM members WHERE usermembername='$user_id' OR Email='$user_id' AND password='$password'";
    $results = mysqli_query($con, $sql);

    if (mysqli_num_rows($results) == 1) {
      $_SESSION['username'] = $user_id;
      $_SESSION['success'] = "You are now logged in";
      header('location: ../passwordmember/success.php');
    }else {
...
if (isset($_POST['reset-password'])) {
  ...
  if (count($errors) == 0) {
    ...
    $msg = "Hi there, click on this <a href=\"localhost/badgesys/passwordmember/new_password.php?email=".$email."&token=".$token."\">link</a> to reset your password on our site";
    $msg = wordwrap($msg,70);
    ...
    header('location: ../passwordmember/pending.php?email=' . $email);
  }
}
...
if (isset($_GET['token']) != $tokens) {
  header('location: ../passwordmember/expire.php?email=' . $email);
  } 
} 
...
if (isset($_POST['new_password'])) {
  ...
  if ($time_expire > $current_time) {
    ...
    if ($email) {
        ...
        $sql = "UPDATE members SET Password='$hashedPassword' WHERE Email='$email' ";
        $results = mysqli_query($con, $sql);
        $deleterecord = "DELETE FROM password_reset WHERE token = '$token'";
        $result = mysqli_query($con, $deleterecord);
        header('location: ../passwordmember/success.php?email=' . $email);
        }        
      }       
  } else {
    $deleterecord = "DELETE FROM password_reset WHERE token = '$token'";
